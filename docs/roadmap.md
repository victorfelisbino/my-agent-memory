...
1. **Write-path integration.** Wire `score_memory.py` into a real local write path (e.g. a pre-commit hook on memory `.md` files, or a wrapper around `capture-observation.ps1`) so the scorer actually filters writes instead of just measuring itself.
2. **Smarter contradiction-against-store / feedback-loop.** Iters 10-11 use polarity+subject-overlap heuristics. Replace with embedding-based similarity once an upstream MCP Memory wrap is chosen (Wave 5-A) so paraphrased contradictions and synonym-swapped re-ingestions with no shared content tokens also fire.
3. **Dashboard slice 2.** Iter 12 shipped a static HTML view of the scoring log; slice 2 = time-series rejection-rate chart + per-source breakdown + staleness view once memories carry timestamps.

**Done so far:**
- Steps 0a-0e above (fixture + PowerShell scorer + Python port + cross-language parity + extractor + iter-10 contradiction-against-store + iter-11 feedback-loop + iter-12 dashboard slice 1 + CI gate at 85% with 15-pt headroom under the current 100% baseline, real-corpus smoke at 1.0% rejection -- all 4 defensible, twelve iteration loops landed with honest before/after numbers per PR; **iter 7 cleared the Wave 3 exit criterion on the labeled fixture for the first time** at 91/100/82; **iter 8 saturated v4 at 100/100/100**; **iter 9 added a standalone Python port** so the scorer is now reusable as middleware outside this repo's CI; **iter 10 wired the novelty dimension to a contradiction-against-store check** (v4 -> v5 fixture growth 100 -> 108: +4 contradicts, +4 reinforces; new `-Store` / `--store` flag; same subject + opposite polarity = -2.0 penalty; 108/108/108 with store on v5 in both languages); **iter 11 extended novelty with a feedback-loop check** (v5 -> v6 fixture growth 108 -> 116: +4 distinct-topic keeps, +4 feedback-loop rejects; new `-Recalled` / `--recalled` flag; same subject + same polarity AND >= 4 shared tokens = -2.0 penalty; 116/116/116 with store + recalled on v6 in both languages; v4 and v5 baselines unchanged; real corpus parity 381/381 with store + recalled); **iter 12 shipped dashboard slice 1** (new `-LogTo` / `--log-to` flag on both scorers; `render-dashboard.ps1` reads the JSONL log and emits a single self-contained `dashboard.html`; CI smoke verifies the loop end-to-end; log + dashboard gitignored, local-only audit tool, no server, no JS framework)).
