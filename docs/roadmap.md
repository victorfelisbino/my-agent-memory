...
- CI job `admission-gate-harness` runs the scorer on every PR with `-FailUnder 95` AND smoke-extracts + scores the live real-memory corpus (~403 items, 0% rejection).
- Current baseline: **100% accuracy, 100% good-recall, 100% junk-recall** on the v2 fixture (40 items). Iteration history: v1 75/100/50 -> iter1 80/100/60 -> iter2 95/100/90 (on v1) -> iter3 100/100/100 (on v1) -> iter4 95/100/90 (v1 -> v2 fixture growth 20 -> 40; +6 rules) -> iter5 100/100/100 (on v2, both v2 misses closed via case-sensitive proper-noun rule + environmental anchor-word rule). **Wave 3 exit criterion is still ahead** -- requires the fixture to be >=100 items.
...
2. **Iter 7 -- v3 -> v4 (60 -> 100 items).** Reach Wave 3 exit-criterion fixture size. Final tuning round.
...
**Done so far:**
- Steps 0a-0d above (fixture + scorer + extractor + CI gate at 95%, real-corpus smoke at 0% rejection, five iteration loops landed with honest before/after numbers per PR; both v2 misses closed in iter 5).
