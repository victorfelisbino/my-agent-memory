...
- [`admission-gate/fixtures/memories-v3.jsonl`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/fixtures/memories-v3.jsonl) — 60 labeled memories (30 keep, 30 reject) covering the documented junk categories plus iter 6 additions: heading-only extraction artifacts, template placeholders with bare TBD/TODO, mixed-purpose bullets, long-undifferentiated dumps, status pings, aspirational vague wishes, vague comparisons, apology / meta-conversation, restated-question shape, numeric heartbeats.
...
- CI job `admission-gate-harness` runs the scorer on every PR with `-FailUnder 95` AND smoke-extracts + scores the live real-memory corpus (~403 items, 5.5% rejection — every reject is a true heading-extraction artifact that should not be a memory; new heading-only rule surfaced a real extractor bug).
- Current baseline: **100% accuracy, 100% good-recall, 100% junk-recall** on the v3 fixture (60 items). Iteration history: v1 75/100/50 -> iter1 80/100/60 -> iter2 95/100/90 (on v1) -> iter3 100/100/100 (on v1) -> iter4 95/100/90 (v1 -> v2 fixture growth 20 -> 40; +6 rules) -> iter5 100/100/100 (on v2, both v2 misses closed via case-sensitive proper-noun rule + environmental anchor-word rule) -> iter6 100/100/100 (v2 -> v3 fixture growth 40 -> 60; raw run was 90/100/80, +6 rules closed heading-only / bare-placeholder / aspirational / vague-comparison / apology-meta / open-question shapes). **Wave 3 exit criterion is still ahead** -- requires the fixture to be >=100 items.

**Still to do (concrete, ordered):**

1. **Iter 7 -- v3 -> v4 (60 -> 100 items).** Reach Wave 3 exit-criterion fixture size. Add shapes the real corpus and v3 round-trip surface: multi-claim bullets that should split into atomic memories, ambiguous-pronoun bullets ("it depends", "this works" with no anchor), domain-mixed bullets that combine two unrelated rules. Expect another accuracy dip; close it with new rules.
2. **Extractor fix.** The iter-6 heading-only rule revealed that `extract-corpus.ps1` is grabbing bullet-style sub-headers as memories. Fix at the source (skip bullets ending in `:` with no content after) so the admission gate is not papering over an upstream bug.
3. **Standalone port.** Lift the rules out of `score-memory.ps1` into a Python or TypeScript module so the scorer can be embedded as middleware in any pipeline (not just CI). Same labeled fixture as the contract; cross-language parity tests in CI.
4. **Contradiction-against-store detection.** Currently we only detect single-line contradictions (`always X; never X`). Wire up a similarity check against a stored memory index so a new candidate can be flagged when it conflicts with something already kept.
5. **Feedback-loop prevention.** Track per-session recall so a memory surfaced during a session cannot be re-ingested as a "new" observation in the same session.
6. **Dashboard.** Local web UI showing: live scoring feed (per-dimension scores + decision + reason), contradiction warnings, feedback-loop blocks, rejection-rate over time, top rejection reasons, staleness view. Reads from the scoring log; no auth, local-only.

**Done so far:**
- Steps 0a-0d above (fixture + scorer + extractor + CI gate at 95%, real-corpus smoke at 5.5% rejection with all rejects being true extractor artifacts, six iteration loops landed with honest before/after numbers per PR; both v2 misses closed in iter 5; all six new v3 misses closed in iter 6).
