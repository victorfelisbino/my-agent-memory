...
2. **Dashboard slice 2.** Iter 12 shipped a static HTML view of the scoring log; slice 2 = time-series rejection-rate chart + per-source breakdown + staleness view once memories carry timestamps. Iter-13 write-path integration now produces the real timestamped capture history that slice 2 will chart.
3. **Rejection-reason review loop.** Now that real captures generate `observations.rejected.jsonl`, periodically review the reject log to catch gate misfires and either fix the rule, add a fixture row, or tighten the bypass UX.

**Done so far:**
- Steps 0a-0e above (fixture + PowerShell scorer + Python port + cross-language parity + extractor + iter-10 contradiction-against-store + iter-11 feedback-loop + iter-12 dashboard slice 1 + iter-13 write-path integration + CI gate at 85% with 15-pt headroom under the current 100% baseline, real-corpus smoke at 1.0% rejection -- all 4 defensible, thirteen iteration loops landed with honest before/after numbers per PR; **iter 7 cleared the Wave 3 exit criterion on the labeled fixture for the first time** at 91/100/82; **iter 8 saturated v4 at 100/100/100**; **iter 9 added a standalone Python port** so the scorer is now reusable as middleware outside this repo's CI; **iter 10 wired the novelty dimension to a contradiction-against-store check** (v4 -> v5 fixture growth 100 -> 108: +4 contradicts, +4 reinforces; new `-Store` / `--store` flag; same subject + opposite polarity = -2.0 penalty; 108/108/108 with store on v5 in both languages); **iter 11 extended novelty with a feedback-loop check** (v5 -> v6 fixture growth 108 -> 116: +4 distinct-topic keeps, +4 feedback-loop rejects; new `-Recalled` / `--recalled` flag; same subject + same polarity AND >= 4 shared tokens = -2.0 penalty; 116/116/116 with store + recalled on v6 in both languages; v4 and v5 baselines unchanged; real corpus parity 381/381 with store + recalled); **iter 12 shipped dashboard slice 1** (new `-LogTo` / `--log-to` flag on both scorers; `render-dashboard.ps1` reads the JSONL log and emits a single self-contained `dashboard.html`; CI smoke verifies the loop end-to-end; log + dashboard gitignored, local-only audit tool, no server, no JS framework); **iter 13 wired the scorer into the actual write path** (new `-ScoreOne` / `--score-one` flag in both scorers; `capture-observation.ps1` / `capture-observation.sh` route every candidate through the gate before appending; rejects divert to `observations.rejected.jsonl` with the reason; bypass via `-NoGate` / `--no-gate` or `MEMORY_GATE=off`; CI smoke covers keep/reject/bypass end-to-end -- **the scorer now filters real writes**, no longer just measures itself)).
