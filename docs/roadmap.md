...
- CI job `admission-gate-harness` runs the scorer on every PR with `-FailUnder 90` AND smoke-extracts + scores the live real-memory corpus.
- Current baseline: **95% accuracy, 100% good-recall, 90% junk-recall** (iter 2 — lower baseline rewards, stronger tautology penalty, new heartbeat / liveness-noise pattern). Clears the 80% percentage threshold on all three. **Not yet at the Wave 3 exit criterion** — that also requires the fixture to be ≥100 items (today: 20). Iteration history: v1 75/100/50 → iter1 80/100/60 → iter2 95/100/90. One documented miss remains (contradiction-shape `always X; never X` partially offset by verb credit) — the next iteration's only target.
