...
3. **Rejection-reason review loop.** Now that real captures generate `observations.rejected.jsonl` from both the manual and the automated path, periodically review the reject log to catch gate misfires and either fix the rule, add a fixture row, or tighten the bypass UX.
4. **Sync-time re-scoring (deferred / decision pending).** `sync-memory.ps1` pulls observations from other machines via `.gitattributes merge=union`, which trusts upstream gating. Acceptable as long as every machine runs a gated capture path (now true after iter 14). Revisit if probe data shows cross-machine contamination.
