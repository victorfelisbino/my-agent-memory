...
1. **Feedback-loop prevention.** Track per-session recall so a memory surfaced during a session cannot be re-ingested as a "new" observation in the same session.
2. **Dashboard.** Local web UI showing: live scoring feed (per-dimension scores + decision + reason), contradiction warnings, feedback-loop blocks, rejection-rate over time, top rejection reasons, staleness view. Reads from the scoring log; no auth, local-only.
3. **Smarter contradiction-against-store.** Iter 10 uses a polarity+subject-overlap heuristic. Replace with an embedding-based similarity check once an upstream MCP Memory wrap is chosen (Wave 5-A) so paraphrased contradictions with no shared content tokens also fire.

**Done so far:**
- Steps 0a-0e above (fixture + PowerShell scorer + Python port + cross-language parity + extractor + iter-10 contradiction-against-store + CI gate at 85% with 15-pt headroom under the current 100% baseline, real-corpus smoke at 1.0% rejection -- all 4 defensible, ten iteration loops landed with honest before/after numbers per PR; **iter 7 cleared the Wave 3 exit criterion on the labeled fixture for the first time** at 91/100/82; **iter 8 saturated v4 at 100/100/100**; **iter 9 added a standalone Python port** so the scorer is now reusable as middleware outside this repo's CI; **iter 10 wired the novelty dimension to a contradiction-against-store check** (v4 -> v5 fixture growth 100 -> 108: +4 contradicts, +4 reinforces; new `-Store` / `--store` flag; same subject + opposite polarity = -2.0 penalty; 108/108/108 with store on v5 in both languages; v4 baseline unchanged at 100/100/100 without `-Store`; real corpus parity 381/381 with and without store)).
