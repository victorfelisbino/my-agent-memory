...
- GitHub Actions cron that regenerates the map daily and commits if anything changed. (The weekly-loop wiring originally planned here was mooted by the June 2026 retirement of `run-weekly-memory.ps1`.)
- README pointer.
...
- [`admission-gate/fixtures/memories-v4.jsonl`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/fixtures/memories-v4.jsonl) — 100 labeled memories (50 keep, 50 reject) covering the documented junk categories plus iter-6 additions plus iter-7 additions (date-only stamp, status-update ping, self-reminder, pop-culture reference, confidence-only, task-restatement, empty agreement, hedge stacking, tool-availability ping, personal scheduling, greeting / sign-off, self-praise, vague urgency, acronym soup, number-only summary, side-note off-topic, user-speculation, restated docs, imperative-only short, self-correction loop). **Wave 3 exit-criterion size.**
...
3. **Rejection-reason review loop.** Periodically review any `*.rejected.jsonl` log produced by a gated pipeline to catch gate misfires and either fix the rule, add a fixture row, or tighten the bypass UX. (The repo's own capture scripts that produced these logs were retired in June 2026; the loop applies to whatever pipeline embeds the `--score-one` contract.)
4. **Sync-time re-scoring (mooted).** `sync-memory.ps1` was retired in June 2026 along with the rest of the capture/sync pipeline, so cross-machine merge trust is no longer this repo's concern. If an external pipeline syncs gated stores across machines, re-apply the same principle: every writer runs the gate.

**Done so far:**
- Steps 0a-0e above (fixture + PowerShell scorer + Python port + cross-language parity + extractor + iter-10 contradiction-against-store + iter-11 feedback-loop + iter-12 dashboard slice 1 + iter-13 manual write-path integration + iter-14 auto-capture write-path integration + CI gate at 85% with 15-pt headroom under the current 100% baseline, real-corpus smoke at 1.0% rejection -- all 4 defensible, fourteen iteration loops landed with honest before/after numbers per PR; **iter 7 cleared the Wave 3 exit criterion on the labeled fixture for the first time** at 91/100/82; **iter 8 saturated v4 at 100/100/100**; **iter 9 added a standalone Python port** so the scorer is now reusable as middleware outside this repo's CI; **iter 10 wired the novelty dimension to a contradiction-against-store check** (v4 -> v5 fixture growth 100 -> 108: +4 contradicts, +4 reinforces; new `-Store` / `--store` flag; same subject + opposite polarity = -2.0 penalty; 108/108/108 with store on v5 in both languages); **iter 11 extended novelty with a feedback-loop check** (v5 -> v6 fixture growth 108 -> 116: +4 distinct-topic keeps, +4 feedback-loop rejects; new `-Recalled` / `--recalled` flag; same subject + same polarity AND >= 4 shared tokens = -2.0 penalty; 116/116/116 with store + recalled on v6 in both languages; v4 and v5 baselines unchanged; real corpus parity 381/381 with store + recalled); **iter 12 shipped dashboard slice 1** (new `-LogTo` / `--log-to` flag on both scorers; `render-dashboard.ps1` reads the JSONL log and emits a single self-contained `dashboard.html`; CI smoke verifies the loop end-to-end; log + dashboard gitignored, local-only audit tool, no server, no JS framework); **iter 13 wired the scorer into the manual write path** (new `-ScoreOne` / `--score-one` flag in both scorers; `capture-observation.ps1` / `capture-observation.sh` route every candidate through the gate before appending; rejects divert to `observations.rejected.jsonl` with the reason; bypass via `-NoGate` / `--no-gate` or `MEMORY_GATE=off`; CI smoke covers keep/reject/bypass end-to-end); **iter 14 closed the auto-capture bypass** -- the iter-13 narrative "the scorer now filters real writes" was only true for the manual path. A roadmap audit caught that `auto-capture-observations.ps1` / `.sh` (the volume path, invoked weekly by `run-weekly-memory.ps1` and on every sync by `sync-memory.ps1`) still appended directly. Iter 14 wires the same per-candidate `--score-one` gate into both auto-capture scripts; same divert-to-`observations.rejected.jsonl` (with `source: auto-capture` tag); same `MEMORY_GATE=off` / `-NoGate` / `--no-gate` bypass; CI smoke covers PS gated, PS bypass, and shell gated runs against a synthetic transcript. **The whole write path is now gated**, not just the manual one.).
