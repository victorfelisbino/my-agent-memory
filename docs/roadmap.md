...
- CI job `admission-gate-harness` runs the scorer on every PR with `-FailUnder 100` AND smoke-extracts + scores the live real-memory corpus (~403 items, 0% rejection).
- Current baseline: **100% accuracy, 100% good-recall, 100% junk-recall** (iter 3 — contradiction-shape now uses phrase-overlap detection so `always X; never X` is caught while `always X; never Y` is not). Iteration history: v1 75/100/50 → iter1 80/100/60 → iter2 95/100/90 → iter3 100/100/100. **This is not the exit criterion** — 100% on a 20-item fixture is an overfitting warning. The next step is to grow the fixture toward 100 items with new junk shapes drawn from real engineering memory, which will deliberately push these numbers back down so the actual blind spots surface.
