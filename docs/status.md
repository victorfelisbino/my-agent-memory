...
- **Admission-gate measurement harness (Wave 3, v1 + 3 iterations).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 20-item labeled fixture, a corpus extractor that pulls real bullets from this repo's own .md files, and a baseline stub scorer across the four Wave 3 dimensions with both labeled-accuracy and unlabeled-distribution modes. Current baseline on the v1 fixture: **100% accuracy, 100% good-recall, 100% junk-recall** — but this is a known overfitting warning, not a finish line: a 20-item fixture isn't the Wave 3 exit criterion (≥100 items required), and the scorer was tuned against this exact fixture across three iterations. The next milestone is to grow the fixture so the numbers drop and the real blind spots become visible. CI runs the scorer on every PR with `-FailUnder 100` and also smoke-extracts the live real-memory corpus (~403 items) to verify no actual memory gets rejected. No write-path integration, no dashboard, no novelty lookup yet.
