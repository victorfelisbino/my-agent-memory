...
- **Admission-gate measurement harness (Wave 3, v4 fixture + 9 iterations, both PowerShell and Python scorers with cross-language parity).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 100-item labeled fixture (50 keep, 50 reject), a corpus extractor, and a baseline stub scorer in both [`score-memory.ps1`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/score-memory.ps1) and [`score_memory.py`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/score_memory.py). Current baseline on v4: **100% accuracy, 100% good-recall, 100% junk-recall in both languages**. Iter 7 cleared the Wave 3 exit criterion for the first time at 91/100/82; iter 8 closed the nine named misses and saturated v4 at 100/100/100; iter 9 lifted the rules into a standalone Python port so the scorer can be embedded as middleware outside this repo's PowerShell-native CI. Cross-language parity is enforced in CI by [`parity-check.ps1`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/parity-check.ps1): same fixture in, same per-item keep/reject out. Today CI verifies parity on both v4 (100 items) and the live real-memory corpus (381 items); a one-sided rule change will fail the build. Real-corpus rejection is 1.0% (4 of 381 items: 1 Status snapshot + 3 personal-goal TODO bullets, all defensible). No write-path integration, no dashboard, no novelty lookup yet -- a Python-callable scorer is necessary but not sufficient.
