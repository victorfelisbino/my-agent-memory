...
- **Admission-gate measurement harness (Wave 3, v4 fixture + 8 iterations, exit criterion met with full headroom on the labeled fixture).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 100-item labeled fixture (50 keep, 50 reject), a corpus extractor that pulls real bullets from this repo's own .md files (skipping heading-only sub-bullets so it stops emitting extraction artifacts), and a baseline stub scorer across the four Wave 3 dimensions with both labeled-accuracy and unlabeled-distribution modes. Current baseline on v4: **100% accuracy, 100% good-recall, 100% junk-recall**. Iter 7 cleared the Wave 3 exit criterion for the first time at 91/100/82; iter 8 closed the nine named misses (pop-culture, confidence-only, task-restatement, empty-agreement, self-praise, vague-urgency, number-only summary, imperative-only short, self-correction loop) with nine small rules and no keep regressions, saturating v4. CI runs the scorer on every PR with `-FailUnder 85` (15-pt headroom; deliberately loose) and also smoke-extracts the real-memory corpus (~381 items); real-corpus rejection is 1.0% (4 items: 1 `Status: ...` playbook snapshot + 3 personal-goal TODO bullets from `goals.md` correctly flagged as imperative-only-short -- all defensible). No write-path integration, no dashboard, no novelty lookup yet -- saturating a synthetic fixture is necessary but not sufficient. The next leverage step is a standalone port (Python or TypeScript) so the scorer can be embedded as middleware in any pipeline, not just this repo's CI.
