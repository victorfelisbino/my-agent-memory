...
- **Admission-gate measurement harness (Wave 3, v4 + v5 fixtures + 10 iterations, both PowerShell and Python scorers with cross-language parity).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 100-item labeled fixture (50 keep, 50 reject), an iter-10 v5 extension (108 items: v4 + 4 contradicts-store + 4 reinforces-store), a 6-anchor store fixture, a corpus extractor, and a baseline stub scorer in both [`score-memory.ps1`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/score-memory.ps1) and [`score_memory.py`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/score_memory.py). Current baseline: **100/100/100 on v4 (no store)** and **108/108/108 on v5 (with `-Store`)** in both languages. Iter 7 cleared the Wave 3 exit criterion for the first time at 91/100/82; iter 8 closed the nine named misses and saturated v4 at 100/100/100; iter 9 lifted the rules into a standalone Python port so the scorer can be embedded as middleware outside this repo's PowerShell-native CI; iter 10 wired the previously-stubbed novelty dimension to a contradiction-against-store check (polarity + subject-overlap; -2.0 when same subject + opposite polarity vs any anchor). Cross-language parity is enforced in CI by [`parity-check.ps1`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/parity-check.ps1): same fixture in, same per-item keep/reject out, with or without `-Store`. Today CI verifies parity on v4 (100 items), v5 + store (108 items), the live real-memory corpus (381 items, no store), and the real-memory corpus with store; a one-sided rule change will fail the build. Real-corpus rejection is 1.0% (4 of 381 items: 1 Status snapshot + 3 personal-goal TODO bullets, all defensible), unchanged by the iter-10 store check. No write-path integration and no dashboard yet -- a Python-callable scorer with a contradiction check is necessary but not sufficient.
