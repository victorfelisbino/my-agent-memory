...
- **Admission-gate measurement harness (Wave 3, v4 fixture + 7 iterations, exit criterion met on the labeled fixture).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 100-item labeled fixture (50 keep, 50 reject), a corpus extractor that pulls real bullets from this repo's own .md files (now skipping heading-only sub-bullets so it stops emitting extraction artifacts), and a baseline stub scorer across the four Wave 3 dimensions with both labeled-accuracy and unlabeled-distribution modes. Current baseline on v4: **91% accuracy, 100% good-recall, 82% junk-recall** -- the first run that credibly clears the Wave 3 exit criterion (>=80% on both recalls, fixture >=100 items). Raw run of the iter-6 ruleset against v4 was 82/100/64; iter 7 closed nine of the 18 new miss shapes with eight small rules (status-update-ping / self-reminder / hedge-stacking / personal-scheduling / greeting-signoff / side-note / user-speculation / restated-docs). The remaining 9 misses are named iter-8 targets (pop-culture, confidence-only, task-restatement, empty-agreement, self-praise, vague-urgency, number-only summary, imperative-only short, self-correction loop) -- documented, not hidden. CI runs the scorer on every PR with `-FailUnder 85` (6-pt headroom below the current 91% baseline) and also smoke-extracts the real-memory corpus (~381 items now that headings are dropped at the source); real-corpus rejection is 0.3% (1 item, a defensible `Status: ...` project snapshot). No write-path integration, no dashboard, no novelty lookup yet -- meeting the exit criterion on the labeled fixture is necessary but not sufficient.
