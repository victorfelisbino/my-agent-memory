...
- **Admission-gate measurement harness (Wave 3, v1 + 2 iterations).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 20-item labeled fixture, a corpus extractor that pulls real bullets from this repo's own .md files, and a baseline stub scorer across the four Wave 3 dimensions with both labeled-accuracy and unlabeled-distribution modes. Current baseline on the v1 fixture: **95% accuracy, 100% good-recall, 90% junk-recall** — all three above the 80% percentage threshold, but the Wave 3 exit criterion also requires the fixture to be ≥100 items (today: 20), so this is "on track," not "done." One documented miss remains (contradiction-shape `always X; never X`). CI runs the scorer on every PR with `-FailUnder 90` and also smoke-extracts the live real-memory corpus to verify nothing in actual memory is getting rejected. No write-path integration, no dashboard, no novelty lookup yet.
