...
- **Admission-gate measurement harness (Wave 3, v1).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 20-item labeled fixture and a baseline stub scorer across the four Wave 3 dimensions. Current baseline on the v1 fixture: 75% accuracy, 100% good-recall, 50% junk-recall — above 50% random, below the 80%+ exit bar. CI runs the scorer on every PR with `-FailUnder 70` so accuracy can't silently regress. No write-path integration, no dashboard, no novelty lookup yet.
