...
- **Admission-gate measurement harness (Wave 3, v1 + iteration 1).** [`admission-gate/`](https://github.com/victorfelisbino/my-agent-memory/blob/main/admission-gate/) holds a 20-item labeled fixture, a corpus extractor that pulls real bullets from this repo's own .md files, and a baseline stub scorer across the four Wave 3 dimensions with both labeled-accuracy and unlabeled-distribution modes. Current baseline on the v1 fixture: 80% accuracy, 100% good-recall, 60% junk-recall — above 50% random, below the 80%+ exit bar. CI runs the scorer on every PR with `-FailUnder 75` so accuracy can't silently regress, and also smoke-extracts the real corpus to verify nothing in actual memory is getting rejected. No write-path integration, no dashboard, no novelty lookup yet.
